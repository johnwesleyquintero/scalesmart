...
interface KeywordPerformanceOverviewTableProps {
...
const KeywordPerformanceOverviewTable: React.FC<
  KeywordPerformanceOverviewTableProps
> = ({ metrics, searchTerm }) => {
  ...
  const [localKeywordFilter, setLocalKeywordFilter] = useState(''); // New state for local keyword filter
    ...
    return metrics
      .filter(
        (metric) =>
          ...
          (localKeywordFilter === '' || // Apply local keyword filter
            metric.targeted_keyword
              ?.toLowerCase()
              .includes(localKeywordFilter.toLowerCase())) &&
          (asinFilter === '' ||
  ...
  }, [metrics, searchTerm, asinFilter, localKeywordFilter]); // Add localKeywordFilter to dependencies
      ...
      <div className="p-4">
        <div className="flex space-x-4 mb-4">
          ...
          <Input
            ...
            value={localKeywordFilter} // Use local keyword filter state
            onChange={(e) => setLocalKeywordFilter(e.target.value)} // Update local keyword filter state
            className="max-w-xs"
          />
        </div>
...
export default KeywordPerformanceOverviewTable;
