...
/**
 * Interface for the overall result of processing a CSV file.
 */
export interface CsvProcessingResult {
  metrics: DashboardMetrics[];
  errors: TransformationError[];
  totalRows: number;
  processedRows: number; // Number of rows attempted to process (excluding header/empty)
  validRows: number; // Number of rows successfully transformed into DashboardMetrics
}

// --- Internal Helper Functions (adapted from data-transformation.ts and csv-utils.ts) ---
...
/**
 * Finds, parses, and validates the first valid numeric value from a list of potential headers in a CSV row.
 * Collects errors for invalid formats and flags potential outliers.
 * @param row The raw CSV row data.
 * @param mappedHeader The primary mapped header key.
 * @param fallbackHeaders List of fallback headers to check in order.
 * @param rowNumber The 0-indexed row number.
 * @param errors Array to push errors/warnings into.
 * @param targetMetricsConfig The configuration for target metrics.
 * @returns An object containing the found numeric value (or undefined), whether a value was found, the raw value used, and an outlier flag.
 */
const getAndValidateNumericValue = (
  row: Record<string, string>,
  mappedHeader: keyof DashboardMetrics | null | undefined,
  ...
  for (const header of headersToCheck) {
    ...
    if (rawValue !== undefined && rawValue !== null) {
      ...
      if (!isNaN(num)) {
        numericValue = num;
        // Check for outliers if this is the primary mapped header and a valid number was found
        if (
          mappedHeader &&
          mappedHeader !== 'Unknown' &&
          numericValue !== undefined &&
          isOutlier(mappedHeader, numericValue)
        ) {
          outlierFlag = true;
          const metricConfig = targetMetricsConfig.find(
            (config) => config.key === mappedHeader,
  ...
  return { value: numericValue, found: foundValue, rawValueUsed, outlierFlag };
};
...
const transformCsvRow = (
  ...
  const total_sales_result = getAndValidateNumericValue(
    row,
    mapping.total_sales,
  ...
  const total_orders_result = getAndValidateNumericValue(
    row,
  ...
  const total_sessions_result = getAndValidateNumericValue(
    row,
  ...
  const total_page_views_result = getAndValidateNumericValue(
    row,
  ...
  const ad_impressions_result = getAndValidateNumericValue(
    row,
  ...
  const ad_clicks_result = getAndValidateNumericValue(
    row,
    mapping.ad_clicks,
    ['Clicks'],
    rowNumber,
  ...
  const ad_spend_result = getAndValidateNumericValue(
    row,
    mapping.ad_spend,
    ['Spend', 'Cost'],
    rowNumber,
  ...
  const ad_sales_result = getAndValidateNumericValue(
    row,
    mapping.ad_sales,
  ...
  const ad_orders_result = getAndValidateNumericValue(
    row,
    mapping.ad_orders,
  ...
  const profit_result = getAndValidateNumericValue(
    row,
    mapping.profit,
    [],
    rowNumber,
  ...
  const inventory_level_result = getAndValidateNumericValue(
    row,
  ...
  const review_rating_result = getAndValidateNumericValue(
    row,
  ...
  const cac_result = getAndValidateNumericValue(
    row,
    mapping.cac,
    [],
    rowNumber,
  ...
  const ltv_result = getAndValidateNumericValue(
    row,
    mapping.ltv,
    [],
    rowNumber,
  ...
  const keyword_ad_impressions_result = getAndValidateNumericValue(
    row,
  ...
  const keyword_ad_clicks_result = getAndValidateNumericValue(
    row,
  ...
  const keyword_ad_spend_result = getAndValidateNumericValue(
    row,
  ...
  const keyword_ad_sales_7_day_result = getAndValidateNumericValue(
    row,
  ...
  const keyword_ad_orders_7_day_result = getAndValidateNumericValue(
    row,
  ...
  const rawTotalConversionRate =
    total_sessions_result.value !== undefined &&
    total_sessions_result.value > 0 &&
    total_orders_result.value !== undefined
  ...
  const acos =
    ad_spend_result.value !== undefined &&
    ad_sales_result.value !== undefined &&
    ad_sales_result.value > 0
      ? (ad_spend_result.value / ad_sales_result.value) * 100
      : 0;
  const roas =
    ad_spend_result.value !== undefined &&
    ad_spend_result.value > 0 &&
    ad_sales_result.value !== undefined
      ? ad_sales_result.value / ad_spend_result.value
      : 0;
  const cpc =
    ad_spend_result.value !== undefined &&
    ad_clicks_result.value !== undefined &&
    ad_clicks_result.value > 0
      ? ad_spend_result.value / ad_clicks_result.value
      : 0;
  const ctr =
    ad_impressions_result.value !== undefined &&
  ...
  const ad_conversion_rate =
    ad_clicks_result.value !== undefined &&
    ad_clicks_result.value > 0 &&
    ad_orders_result.value !== undefined
...
/**
 * Processes a CSV file using a provided column mapping and target metric configuration.
 * Parses, transforms, and validates each row, collecting errors and returning processed metrics.
 * @param file The CSV File object to process.
 * @param mapping The CsvColumnMapping object.
 * @param targetMetricsConfig The configuration for target metrics.
 * @param onProgress Optional callback function to report processing progress (0-100).
 * @returns A promise resolving to a CsvProcessingResult object.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    Papa.parse<Record<string, string>>(file, {
      ...
      step: (rowParseResult, parser) => {
        processedRows++;
        if (onProgress) {
          // Estimate progress based on file size or row count if available
          // For simplicity, using row count here, but file size is more accurate for large files
          // Need total file size or total rows beforehand for accurate percentage
          // For now, a simple incrementing counter:
          onProgress(
            Math.min(
              99,
              Math.round((processedRows / (totalRows || processedRows)) * 100),
            ),
          );
        }

        if (rowParseResult.data) {
          const result = transformCsvRow(
            rowParseResult.data,
            mapping,
            processedRows - 1, // Adjust row number to be 0-indexed
            targetMetricsConfig,
          );
      ...
      complete: (results) => {
        totalRows = processedRows; // In step, processedRows is the total count
        if (onProgress) {
...
/**
 * Parses the header and first data row of a CSV file.
 * Useful for displaying mapping options to the user before full processing.
 * @param file The CSV File object.
 * @returns A promise resolving to an object containing headers and the first data row.
 */
export const parseCsvHeadersAndFirstRow = async (
  file: File,
): Promise<{
  headers: string[];
  ...
  return new Promise((resolve, reject) => {
    Papa.parse<Record<string, string>>(file, {
      header: true,
      preview: 1, // Only parse enough to get headers and first data row
      skipEmptyLines: true,
      complete: (results) => {
