...
export default async function ProjectManagementLayout({
  ...
  if (error) {
    console.error(
      'Error fetching user or profile for project management page:',
      error.message,
    );
  ...
  // Check if user exists and has the 'manager', 'editor', or 'admin' role
  const allowedRoles = ['manager', 'editor', 'admin'];
  if (!user || !profile || !allowedRoles.includes(profile.role)) {
    // Redirect to login or an access denied page
    redirect(
      '/login?message=You do not have permission to access Project Management.',
    );
  }
