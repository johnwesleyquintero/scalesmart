  ...
  try {
    ...
    if (!validationResult.success) {
      ...
      return NextResponse.json(
        createErrorResponse(`Invalid input: ${errorMessages}`, 'VALIDATION_ERROR'),
        { status: 400 },
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResult = moduleProgressSchema.partial().safeParse(queryParams);
    ...
    if (!userId || !courseId || !moduleId) {
      return NextResponse.json(
        createErrorResponse('Missing required query parameters (userId, courseId, moduleId)', 'VALIDATION_ERROR'),
        { status: 400 },
      );
    }
