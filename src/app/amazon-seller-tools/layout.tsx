...
export default async function AmazonSellerToolsLayout({
  ...
  if (error) {
    console.error(
      'Error fetching user or profile for Amazon Seller Tools:',
      error.message,
    );
  ...
  // Check if user exists and has the 'seller' or 'admin' role
  const allowedRoles = ['seller', 'admin'];
  if (!user || !profile || !allowedRoles.includes(profile.role)) {
    // Redirect to login or an access denied page
    redirect(
      '/login?message=You do not have permission to access the Amazon Seller Tools.',
    );
  }
