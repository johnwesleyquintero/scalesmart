...
- **Professional:** Maintain a formal yet approachable tone suitable for business users.
- **Reliable:** Convey trustworthiness and stability through clear and consistent language.
- **Helpful:** Provide guidance and support through instructional and informative text.
- **Clear:** Use simple, direct language that is easy to understand and avoids jargon.
...
- **Specific Color Tokens:** Utilize standard Tailwind CSS color names and scales (e.g., `primary-500`, `gray-200`).
- **Semantic Color Mapping:** Map semantic colors to appropriate Tailwind classes or custom CSS variables for semantic meaning (e.g., `text-green-500` for success, `text-red-500` for error).
- **Neutral Color Scale:** Define the grayscale or neutral color scale using Tailwind's default gray scale (e.g., `gray-50`, `gray-100`, ..., `gray-900`).
...
- **Font Families:** Use the primary font stack configured in the project's Tailwind CSS (e.g., `sans-serif`).
- **Font Sizes:** Define sizes using Tailwind's `text-` classes (e.g., `text-sm`, `text-base`, `text-lg`) or `rem` values.
- **Font Weights:** Specify usage of different weights using Tailwind's `font-` classes (e.g., `font-normal`, `font-medium`, `font-bold`).
- **Line Heights:** Define recommended line heights using Tailwind's `leading-` classes (e.g., `leading-normal`, `leading-relaxed`).
- **Letter Spacing:** Define guidelines for letter spacing using Tailwind's `tracking-` classes.
- **Paragraph Spacing:** Define guidelines for paragraph spacing using Tailwind's margin classes (e.g., `mb-4`).
...
- **Icon Library:** Lucide React (based on project file structure).
- **Standard Sizes:** Define standard icon sizes using Tailwind's size scale or `px` values (e.g., `size-4` (16px), `size-5` (20px), `size-6` (24px)).
- **Usage Guidelines:** Provide guidelines for using icons with or without text, ensuring appropriate sizing and alignment within components.
...
- **Base Unit:** Tailwind CSS uses a 4px base unit for its default spacing scale.
- **Spacing Scale:** Utilize Tailwind's predefined spacing scale classes (e.g., `p-1` (4px), `m-2` (8px), `gap-4` (16px)).
- **Application:** Provide guidelines for applying spacing to margins (`m-` classes), padding (`p-` classes), and between elements in flexbox or grid layouts (`gap-` classes).
...
- **Elevation Levels:** Define different levels of elevation using Tailwind's `shadow-` classes (e.g., `shadow-sm`, `shadow-md`, `shadow-lg`).
- **Shadow Properties:** Specify shadow properties as defined by the corresponding Tailwind `shadow-` classes.
...
- **Border Styles:** Define border width (Tailwind's `border-` classes, e.g., `border`, `border-2`), style (Tailwind's `border-solid`), and color (Tailwind's color classes, e.g., `border-gray-300`).
- **Divider Usage:** Provide guidelines for using dividers, typically implemented using borders or dedicated divider components, to visually separate content sections.
